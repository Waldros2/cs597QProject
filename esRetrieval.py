...
for index in indices:
    print('Collecting ' + config['search_type'] + ' data from ' + index)
    connect_dict = defaultdict(float)
    try:
        res = es.search(index=index, q=config['query'], size=10000, scroll='1m')
        while len(res['hits']['hits']) > 0:
            for hit in res['hits']['hits']:
                #Collecting netflow data
                if(config['search_type'] == 'byte'):
        ...
        with open(config['save_path'] + config['search_type']+ '_' + index + '.csv', 'w') as outfile:
            writer = csv.writer(outfile)
            writer.writerow(['src', 'dst', config['search_type']])
